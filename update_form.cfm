...
<table cellpadding="5" border="1">
	<form name="catalog" action="update_form.cfm?method=1" method="post">
      <tr>
          <td>
              Catalog to Update:
          </td>
...
<form name="catalog" action="update_form.cfm?method=2" method="post">
        ...
        <cfquery name="ins" datasource="#application.dsn#">
            INSERT INTO dom_tracks
                (cid, track_title, filename, composer, bpm,
                    instrumentation, cat, format, com_PRO, pub_PRO, publisher_name, filepath, filesize, filetype, duration, vol, volume_title)
            VALUES
                (1, '#trim(song_title)#', '#shock.name#', 'Dominic T. Kelly', '#replace(right(replace(shock.name,".mp3",""),3),"_","")#',
                     ' ', '#catalog#',
                    'mp3', 'ASCAP', 'ASCAP', 'Screw The Pooch', 'mp3s/#filepath#/', '#NumberFormat(size_mb,"9.9")#','#filetype#','#int(seconds / 60)#:#NumberFormat(seconds % 60,"09")#','#volume#', '#cat.catalog_title#')     
        </cfquery>
